...
# Lifetime for presigned URLs returned to browsers for user downloads.
# Matches the skill download presign TTL: long enough for a single download
# attempt, short enough to limit accidental link sharing.
_USER_DOWNLOAD_URL_TTL_SECONDS = 10 * 60
...
def _try_redirect_user_download_to_presigned_url(
    db: Session,
    *,
    attachment_id: int,
    context,
) -> Optional[RedirectResponse]:
    """Return a 302 to a public presigned GET URL when S3 can serve the file.

    Falls back to ``None`` so callers stream bytes through the backend for
    MySQL storage, encrypted attachments, or when presign generation fails.
    """
    if not (
        is_external_storage_configured()
    ...
    presigned_url = backend.get_url(
        context.storage_key,
        expires=_USER_DOWNLOAD_URL_TTL_SECONDS,
        public=True,
    )
    ...
    logger.info(
        "Redirecting user download to presigned URL: "
...
async def download_attachment(
    ...
    Returns:
        Redirect to a public presigned S3 URL when object storage is used,
        otherwise file binary data with appropriate content type
    """
    has_access = False
    context = None
    ...
    redirect = _try_redirect_user_download_to_presigned_url(
        db,
...
async def public_download_attachment(
    ...
    if redirect is not None:
        logger.info(
            "[PublicDownload] User %d redirected to presigned URL for attachment %d",
            current_user.id,
            attachment_id,
        )
        return redirect

...
const SECTION_CONTENT: Record<string, string> = {
  s1: `## 第一章 总体技术方案\n\n本项目采用"云-边-端"三层协同架构，构建统一的智慧园区数字底座。总体架构自下而上分为感知层、网络层、平台层与应用层，各层之间通过标准化接口解耦，保证系统的先进性、开放性与可扩展性。\n\n### 1.1 系统总体架构\n平台层基于微服务架构设计，采用容器化部署与服务网格治理，支持弹性伸缩与灰度发布。\n\n### 1.2 关键技术选型\n- 应用框架：Spring Cloud 微服务体系\n- 数据存储：分布式关系库 + 时序库 + 对象存储\n- 物联网接入：MQTT / CoAP 双协议网关`,
  s2: `## 第二章 数据安全与等级保护\n\n本方案严格响应网络安全等级保护三级（等保 2.0）要求，从物理、网络、主机、应用、数据五个维度构建纵深防御体系。\n\n全链路数据采用国密 SM4 加密传输与存储，结合基于角色的访问控制（RBAC）与最小权限原则，确保敏感数据不泄露、不篡改。`,
  s3: `## 第三章 项目实施与进度管理\n\n项目实施划分为需求调研、设计开发、集成测试、试运行、验收交付五个阶段，总工期 180 日历天。\n\n通过 WBS 工作分解结构明确各阶段交付物与里程碑，采用敏捷迭代与周例会机制动态跟踪进度与风险。`,
  s4: `## 第四章 运维与售后服务\n\n我方承诺提供 7×24 小时运维响应服务，一般故障 30 分钟内响应、2 小时内到场，重大故障启动应急预案并 4 小时内恢复。\n\n配备专职驻场运维团队，建立分级服务台与知识库，保障系统长期稳定运行。`,

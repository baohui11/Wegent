...
## Post-implementation fidelity verification (Task 9)

Reviewer constraint #2 required a real browser round-trip smoke because Jest mocks tiptap. What was verified vs. what requires a human-in-the-browser check:

**Verified programmatically:**
- **Compile / module resolution (caught a real bug):** `NEXT_PUBLIC_BID_MOCK=1 pnpm dev` initially returned HTTP 500 on `/bid-workbench` with `Module not found: Can't resolve '@tiptap/core'` — the new extensions import `@tiptap/core` directly but it was only a transitive dep. Fixed by adding `@tiptap/core@^3.27.1` as a direct frontend dependency. After the fix, the route returns HTTP 200 and all client bundles load with zero runtime errors in the dev log. This is a bug class the Jest suite structurally cannot catch (it mocks `@tiptap/core`).
